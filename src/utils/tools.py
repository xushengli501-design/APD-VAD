...
def resample_feat(feat, target_len):
    ...
    indices = np.rint(np.linspace(0, feat.shape[0] - 1, target_len)).astype(np.int64)
    return feat[indices].astype(np.float32, copy=False)


def split_by_length(feat, length):
    clip_length = feat.shape[0]
    if clip_length < length:
        ...
        for i in range(split_num):
            ...
            if i == 0:
                split_feat = pad(chunk, length).reshape(1, length, feat.shape[1])
            else:
                split_feat = np.concatenate([split_feat, pad(chunk, length).reshape(1, length, feat.shape[1])], axis=0)

        return split_feat.astype(np.float32, copy=False), clip_length
